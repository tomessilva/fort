...
#' `FastTransform` class
#'
#' @description General specification of the type of objects generated by
...
#' Instead, you should use [fort()] and the typical S3 methods for matrices,
#' such as [`%*%.FastTransform`] and [`solve.FastTransform`].
#'
#' @field inverse Logical. Indicates whether the object currently represents
#'   a forward or inverse transform.
#' @field invertible Logical. Indicates whether the inverse transform can also
#'   be expressed as a `FastTransform` object.
...
#' @field blocksize Dimensionality of the internal transformation (always a
#'   power of 2).
...
#' @field fort_type String indicating the type of structured transform being
#'   used.
#' @field cache_matrix Logical. Indicates whether to cache calculated matrices
...
FastTransform <- R6::R6Class(
  ...
  public = list(
    ...
    get_logdet = function() {
      ...
      # check if pre-cached
      if (self$inverse) {
        if (!is.null(logdet$rev)) {
          return(logdet$rev)
        }
      } else {
        if (!is.null(logdet$fwd)) {
          return(logdet$fwd)
        }
      }
    ...
    as_matrix = function() {
      ...
      if (is_inverse) {
        ...
        if (!is.null(self$rev_mtrx)) {
          return(self$rev_mtrx)
        }
      } else {
        # check for cached matrix
        if (!is.null(self$fwd_mtrx)) {
          return(self$fwd_mtrx)
        }
      }
    ...
    summary = function() {
      ...
      is_inverse <- self$inverse
      if (is_inverse && (!self$invertible)) {
        cur_type_line <- "slow linear transform (matrix multiplication)"
      } else {
        cur_type <- self$fort_type
...
#' Validate `FastTransform` Objects
#'
#' Internal function used to verify whether a certain object is a valid and
#' usable object that inherits from `FastTransform`. Thorough, but only used
#' for tests.
#'
...
.is_valid_ft <- function(object) {
  ...
  # all tests passed: object is valid
  out_val <- TRUE
  attr(out_val, "reason") <- "object is valid 'FastTransform'"
  out_val
}
